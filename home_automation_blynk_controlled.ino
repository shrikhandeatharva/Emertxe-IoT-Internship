...
#define BLYNK_TEMPLATE_ID "TMPL3SvSqqwaq"
#define BLYNK_TEMPLATE_NAME "Home automation"
#define BLYNK_AUTH_TOKEN "AxO7HwjHvjTFyXodpxIMV7QZcLGVyvSU"
...
BlynkTimer timer;  // object timer of class blynktimer

LiquidCrystal_I2C lcd(0x27, 16, 2); // set the LCD address to 0x27 for a 16 chars and 2 line display

// This function is called every time the Virtual Pin 0 state changes
/*To turn ON and OFF cooler based virtual PIN value and printing notification on the CLCD*/
...
{
  /* to read the values from the outlet pin */
...
void update_temperature_reading()
{
  ...
  /* reading values from temperature sensor and updating it to virtual pin v1 */
  Blynk.virtualWrite(TEMPERATURE_GAUGE , read_temperature());
  Blynk.virtualWrite(WATER_VOL_GAUGE  , volume());
  /* TEMPEERATURE_GAUGE = V1 PIN */
}
...
void handle_temp(void)
{
  ...
  if((read_temperature() > float(35)) && heater_sw)
  {
    ...
    /* turn off the heater */
    heater_control(OFF);
    /* TO print heater status on the dash board */
    lcd.setCursor(7,0);
      lcd.print("HT_R OFF ");
...
{
  /* to check if the volume of the water is less than 2000 then enable the inlet valve */
  if((tank_volume < 2000) && (inlet_sw == 0))
  {
    ...
    /* refecting the status on the button widget */
   ...
   if((tank_volume == 3000) && (inlet_sw == 1))
  {
    ...
    /* to print status on the dashboard */
     lcd.setCursor(7,1);
      ...
      /* to print the notification on the mobile app */
       Blynk.virtualWrite(BLYNK_TERMINAL_V_PIN , "Volume of water in the tank is full \n");
...
/* no need for paranthesis while setting the callback functon */
}
